...
class MainActivity : AppCompatActivity(), BiljkaClickListener {
    ...
    private val novaBiljkaActivityResult =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                // Fetch the updated plant list from the database
                lifecycleScope.launch {
                    val biljkaDao = BiljkaDatabase.getInstance(applicationContext).biljkaDao()
                    val updatedList = biljkaDao.getAllBiljkas().toMutableList()

                    // Clear the existing list and add the updated list
                    glavnaListaBiljke.clear()
                    glavnaListaBiljke.addAll(updatedList)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Inicijalizacija RecyclerView-a, adaptera i ostalih elemenata UI-a
        ...
        val biljkaDatabase = BiljkaDatabase.getInstance(applicationContext)
        // Inicijalizacija DAO objekta
        val biljkaDao = biljkaDatabase.biljkaDao()
        adapter = BiljkaListAdapter(glavnaListaBiljke, moda, this@MainActivity, glavnaListaBiljke, false)

        // Dohvaćanje svih biljaka iz baze podataka i dodavanje u glavnu listu
        lifecycleScope.launch {
            glavnaListaBiljke = biljkaDao.getAllBiljkas().toMutableList()

            if (glavnaListaBiljke.isNotEmpty()) {
                referentnaBiljka = glavnaListaBiljke[0]
            } else {
                referentnaBiljka = biljke.get(0)
            }

            adapter = BiljkaListAdapter(glavnaListaBiljke, moda, this@MainActivity, glavnaListaBiljke, false)
            recyclerView.adapter = adapter

            // Osiguravanje da se prikaz osvježi nakon učitavanja podataka
            adapter.notifyDataSetChanged()
        }
        //dodao ja
        if(glavnaListaBiljke.isNotEmpty())referentnaBiljka = glavnaListaBiljke[0]
        else referentnaBiljka = biljke.get(0)
    ...
    private fun filterByKuharskiMod(): List<Biljka> {
        return glavnaListaBiljke.filter { biljka ->
            ...
            } || biljka.profilOkusa == referentnaBiljka.profilOkusa || (biljka.jela == null && referentnaBiljka.jela==null)
        }
    }
